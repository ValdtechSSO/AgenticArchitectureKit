...
def _validate_with_schema(instance: Any, schema: dict[str, Any], label: str, source: str) -> None:
...
def _load_catalog(document: Any, source: str, root: Path) -> dict[str, dict[str, Any]]:
    ...
    for index, rule in enumerate(document["rules"]):
        ...
        if not isinstance(rule_id, str) or not rule_id:
            raise ContractError(f"Rule id is invalid at {source}:rules[{index}]")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--toolchain",
        default=".agentic/toolchain.json",
        help="Pinned architecture distribution declaration, relative to root.",
    )
...
def run(arguments: list[str] | None = None) -> int:
    ...
    try:
        ...
        policy_schema = bundled_schema("architecture-policy.schema.json")
        _validate_with_schema(policy, policy_schema, "Architecture policy", "package:architecture-policy.schema.json")
        _validate_with_schema(waiver_document, bundled_schema("architecture-waivers.schema.json"), "Architecture waivers", "package:architecture-waivers.schema.json")
        _validate_with_schema(review_document, bundled_schema("architecture-reviews.schema.json"), "Architecture reviews", "package:architecture-reviews.schema.json")
        ...
        if module_schema_value.startswith("package:"):
            module_schema = bundled_schema(module_schema_value.removeprefix("package:"))
        else:
            module_schema = load_json(_repository_path(root, module_schema_value))
        ...
        report = {
            ...
            "toolchain": _display_path(root, toolchain_path),
        ...
        result_errors = validate_schema(report, bundled_schema("architecture-result.schema.json"))
        if result_errors:
